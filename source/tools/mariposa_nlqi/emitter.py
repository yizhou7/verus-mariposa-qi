...
class Rewriter:
    def __init__(self, name, params):
        ...
        while retry:
            ...
            if len(self.steps) < params.MAX_STEPS:
                retry = True
                self.steps = []
                retry_count += 1
            else:
    ...
    def emit_variables(self, csteps):
    ...
    def emit_as_calc(self, mode, upto, keep_every):
        ...
        lines = self.emit_variables(csteps)
        lines.append("\tcalc !{\n\t\t(==)")
...
#         return lemma + "{}\n"
    ...
    # def emit_asserts(self, mode, KEEP_EVERY=2, upto=None):
    ...
    #         lines.append(f"\tlet {self.mk_temp(i+1)} = {s[1]};")
    #     prev = self.mk_temp(0)

    #     for i, s in enumerate(self.steps[:upto]):
    #         if i % KEEP_EVERY != 0:
    #             lines.append(f"\t\t//assert({prev} == {self.mk_temp(i+1)}) by ")
    #             lines.append("\t\t//\t{" + s[0].emit(mode) + "}\t// " + str(i))
    #         else:
    #             lines.append(f"\t\tassert({prev} == {self.mk_temp(i+1)}) by ")
    #             prev = self.mk_temp(i+1)
    #             lines.append("\t\t\t{" + s[0].emit(mode) + "}\t// " + str(i))
...
class Params:
    ...
    def __str__(self):
        return f"""[INFO] max number of rewrite steps: {self.MAX_STEPS}
...
if __name__ == "__main__":
    ...
    for mode in [StepMode.AUTO_ONLY, StepMode.INST_ONLY]:
        ...
        for rw in rws:
            out_f.write(rw.emit_as_calc(mode, pa.MAX_STEPS, pa.KEEP_EVERY))
            # out_f.write(rw.emit_as_assert(mode, pa.MAX_STEPS, pa.KEEP_EVERY))
