...
;; mariposa_data/2//16170644153679370714/nlqi_verus/src/main.rs:7:1: 16:36 (#0)
(push)
 ...
 (declare-const temp_0_1~122@ Int)
 (declare-const temp_1_0~163@ Int)
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~97@ (Mul (Mul d0~8@ b0~4@) (Add a0~2@ c0~6@)))
     (=>
      (= temp_0_1~122@ (Add (Mul (Mul d0~8@ b0~4@) a0~2@) (Mul (Mul d0~8@ b0~4@) c0~6@)))
      (and
       (=>
        (ens%main!nl_basics.lemma_mul_properties_auto_1. 0)
        (=>
         %%location_label%%0
         (= temp_0_0~97@ temp_0_1~122@)
       ))
       (=>
        (= temp_0_0~97@ temp_0_1~122@)
        (=>
         (= temp_1_0~163@ (Mul (Mul a1~10@ d1~16@) (Sub d1~16@ b1~12@)))
         (=>
          (= temp_1_1~180@ (Mul (Sub d1~16@ b1~12@) (Mul a1~10@ d1~16@)))
          (and
           (=>
            (ens%main!nl_basics.lemma_mul_properties_auto_1. 0)
            (=>
             %%location_label%%1
             (= temp_1_0~163@ temp_1_1~180@)
           ))
           (=>
            (= temp_1_0~163@ temp_1_1~180@)
            (=>
             (= temp_2_0~221@ (Mul (Mul b2~20@ c2~22@) (Mul d2~24@ d2~24@)))
             (=>
              (= temp_2_1~238@ (Mul (Mul d2~24@ d2~24@) (Mul b2~20@ c2~22@)))
              (=>
               (ens%main!nl_basics.lemma_mul_properties_auto_1. 0)
               (=>
                %%location_label%%2
                (= temp_2_0~221@ temp_2_1~238@)
 )))))))))))))))
 (get-info :version)
 (assert
  %%query%%
