...
;; mariposa_data/v_nl//255399699578577713/nlqi_verus/src/main.rs:7:1: 26:40 (#0)
(push)
 ...
 (declare-const temp_1_1~754@ Int)
 (assert
  fuel_defaults
 )
 ;; assertion failed
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~293@ (Mul (Mul (Mul (Sub d0~8@ (Mul a0~2@ d0~8@)) (Mul (Mul a0~2@ c0~6@) (
           Mul 58 a0~2@
         ))
        ) (Mul (Mul (Mul d0~8@ b0~4@) (Mul c0~6@ b0~4@)) (Mul (Sub d0~8@ a0~2@) (Mul d0~8@ b0~4@)))
       ) (Add (Mul (Mul (Mul a0~2@ a0~2@) (Sub c0~6@ a0~2@)) (Mul (Mul d0~8@ b0~4@) (Mul b0~4@
           a0~2@
         ))
        ) (Mul (Sub c0~6@ (Mul d0~8@ c0~6@)) (Mul (Mul d0~8@ b0~4@) (Mul c0~6@ b0~4@)))
     )))
     (=>
      (= temp_0_1~426@ (Mul (Mul (Mul (Sub d0~8@ (Mul a0~2@ d0~8@)) (Mul (Mul a0~2@ c0~6@) (
            Mul 58 a0~2@
          ))
         ) (Mul (Mul (Mul d0~8@ b0~4@) (Mul c0~6@ b0~4@)) (Mul (Sub d0~8@ a0~2@) (Mul d0~8@ b0~4@)))
        ) (Add (Mul (Mul a0~2@ a0~2@) (Mul (Sub c0~6@ a0~2@) (Mul (Mul d0~8@ b0~4@) (Mul b0~4@
             a0~2@
          )))
         ) (Mul (Sub c0~6@ (Mul d0~8@ c0~6@)) (Mul (Mul d0~8@ b0~4@) (Mul c0~6@ b0~4@)))
      )))
      (and
       (=>
        ...
        (=>
         (= tmp%2@ (Sub c0~6@ a0~2@))
         (=>
          (= tmp%3@ (Mul (Mul d0~8@ b0~4@) (Mul b0~4@ a0~2@)))
          (=>
       ...
       (=>
        (= temp_0_0~293@ temp_0_1~426@)
        (=>
         (= temp_1_0~617@ (Mul (Add (Mul (Add (Mul b1~12@ c1~14@) (Mul d1~16@ c1~14@)) (Mul (Mul
               a1~10@ a1~10@
              ) (Add b1~12@ b1~12@)
             )
            ) (Mul (Mul (Mul c1~14@ c1~14@) (Mul d1~16@ d1~16@)) (Add (Mul c1~14@ d1~16@) (Mul a1~10@
               a1~10@
            )))
           ) (Mul (Add (Mul (Mul c1~14@ c1~14@) (Mul d1~16@ c1~14@)) (Mul (Mul d1~16@ d1~16@) (
               Add c1~14@ a1~10@
             ))
            ) (Mul (Mul (Mul b1~12@ c1~14@) (Sub a1~10@ a1~10@)) (Mul (Add b1~12@ 63) (Sub c1~14@
               b1~12@
         ))))))
         (=>
          (= temp_1_1~754@ (Mul (Add (Mul (Mul (Add b1~12@ d1~16@) c1~14@) (Mul (Mul a1~10@ a1~10@)
               (Add b1~12@ b1~12@)
              )
             ) (Mul (Mul (Mul c1~14@ c1~14@) (Mul d1~16@ d1~16@)) (Add (Mul c1~14@ d1~16@) (Mul a1~10@
                a1~10@
          ...
          (=>
           (ens%main!nl_basics.lemma_mul_is_distributive. b1~12@ d1~16@ c1~14@)
           (=>
            %%location_label%%1
            (= temp_1_0~617@ temp_1_1~754@)
 )))))))))))
 (get-info :version)
 (assert
  %%query%%
