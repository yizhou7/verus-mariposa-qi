...
;; mariposa_data/v_nl//1575919775441773124/nlqi_verus/src/main.rs:7:1: 26:40 (#0)
(push)
 ...
 (declare-const temp_1_1~578@ Int)
 (declare-const temp_2_0~729@ Int)
 (declare-const temp_2_1~846@ Int)
 (declare-const temp_3_0~1013@ Int)
 (declare-const temp_3_1~1146@ Int)
 (declare-const temp_4_0~1325@ Int)
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~273@ (Add (Mul (Mul (Mul (Mul b0~4@ b0~4@) (Mul b0~4@ d0~8@)) (Add c0~6@ d0~8@))
        (Mul (Add b0~4@ (Mul a0~2@ a0~2@)) (Mul (Sub a0~2@ a0~2@) (Mul c0~6@ d0~8@)))
       ) (Mul (Mul (Mul (Mul b0~4@ b0~4@) (Add b0~4@ d0~8@)) (Add (Mul d0~8@ d0~8@) d0~8@))
        (Mul (Mul (Sub b0~4@ d0~8@) (Mul b0~4@ d0~8@)) (Mul (Add c0~6@ c0~6@) (Mul d0~8@ a0~2@)))
     )))
     (=>
      (= temp_0_1~402@ (Add (Mul (Mul (Mul (Mul b0~4@ b0~4@) (Mul b0~4@ d0~8@)) (Add c0~6@ d0~8@))
         (Add (Mul b0~4@ (Mul (Sub a0~2@ a0~2@) (Mul c0~6@ d0~8@))) (Mul (Mul a0~2@ a0~2@) (
            Mul (Sub a0~2@ a0~2@) (Mul c0~6@ d0~8@)
         )))
        ) (Mul (Mul (Mul (Mul b0~4@ b0~4@) (Add b0~4@ d0~8@)) (Add (Mul d0~8@ d0~8@) d0~8@))
         (Mul (Mul (Sub b0~4@ d0~8@) (Mul b0~4@ d0~8@)) (Mul (Add c0~6@ c0~6@) (Mul d0~8@ a0~2@)))
      )))
      (and
       (=>
        (= tmp%1@ (Mul a0~2@ a0~2@))
        (=>
         (= tmp%2@ (Mul (Sub a0~2@ a0~2@) (Mul c0~6@ d0~8@)))
         (=>
          (ens%main!nl_basics.lemma_mul_is_distributive. b0~4@ tmp%1@ tmp%2@)
          (=>
           %%location_label%%0
           (= temp_0_0~273@ temp_0_1~402@)
       ))))
       (=>
        (= temp_0_0~273@ temp_0_1~402@)
        (=>
         (= temp_1_0~513@ (Mul d1~16@ (Mul (Sub (Mul (Mul c1~14@ c1~14@) (Mul a1~10@ d1~16@))
             (Mul (Mul b1~12@ a1~10@) (Add b1~12@ d1~16@))
            ) (Mul (Mul (Sub c1~14@ c1~14@) a1~10@) (Mul (Mul c1~14@ d1~16@) (Sub b1~12@ d1~16@)))
         )))
         (=>
          (= temp_1_1~578@ (Mul d1~16@ (Mul (Sub (Mul (Mul c1~14@ c1~14@) (Mul a1~10@ d1~16@))
              (Mul (Mul b1~12@ a1~10@) (Add b1~12@ d1~16@))
             ) (Mul (Mul (Sub c1~14@ c1~14@) a1~10@) (Mul c1~14@ (Mul d1~16@ (Sub b1~12@ d1~16@))))
          )))
          (and
           (=>
            (= tmp%3@ (Sub b1~12@ d1~16@))
            (=>
             (ens%main!nl_basics.lemma_mul_is_associative. c1~14@ d1~16@ tmp%3@)
             (=>
              %%location_label%%1
              (= temp_1_0~513@ temp_1_1~578@)
           )))
           (=>
            (= temp_1_0~513@ temp_1_1~578@)
            (=>
             (= temp_2_0~729@ (Mul (Mul (Mul (Mul (Mul a2~18@ d2~24@) (Mul b2~20@ c2~22@)) (Mul a2~18@
                  (Mul b2~20@ 61)
                 )
                ) (Mul (Sub b2~20@ (Mul a2~18@ a2~18@)) (Mul a2~18@ (Mul b2~20@ b2~20@)))
               ) (Mul (Mul c2~22@ (Mul (Mul c2~22@ b2~20@) (Mul b2~20@ b2~20@))) (Mul (Add (Mul a2~18@
                   b2~20@
                  ) (Mul c2~22@ b2~20@)
                 ) (Mul (Mul c2~22@ c2~22@) (Mul d2~24@ c2~22@))
             ))))
             (=>
              (= temp_2_1~846@ (Mul (Mul (Mul (Mul (Mul a2~18@ d2~24@) (Mul b2~20@ c2~22@)) (Mul a2~18@
                   (Mul b2~20@ 61)
                  )
                 ) (Mul (Sub b2~20@ (Mul a2~18@ a2~18@)) (Mul a2~18@ (Mul b2~20@ b2~20@)))
                ) (Mul (Mul c2~22@ (Mul c2~22@ (Mul b2~20@ (Mul b2~20@ b2~20@)))) (Mul (Add (Mul a2~18@
                    b2~20@
                   ) (Mul c2~22@ b2~20@)
                  ) (Mul (Mul c2~22@ c2~22@) (Mul d2~24@ c2~22@))
              ))))
              (and
               (=>
                (= tmp%4@ (Mul b2~20@ b2~20@))
                (=>
                 (ens%main!nl_basics.lemma_mul_is_associative. c2~22@ b2~20@ tmp%4@)
                 (=>
                  %%location_label%%2
                  (= temp_2_0~729@ temp_2_1~846@)
               )))
               (=>
                (= temp_2_0~729@ temp_2_1~846@)
                (=>
                 (= temp_3_0~1013@ (Mul (Sub (Sub (Mul (Mul d3~32@ b3~28@) (Mul a3~26@ c3~30@)) (Mul (Mul
                       b3~28@ b3~28@
                      ) (Mul d3~32@ d3~32@)
                     )
                    ) (Mul (Sub b3~28@ (Add c3~30@ c3~30@)) (Mul (Mul a3~26@ b3~28@) (Mul a3~26@ c3~30@)))
                   ) (Mul (Mul (Mul b3~28@ (Mul c3~30@ a3~26@)) (Add (Mul a3~26@ c3~30@) (Mul c3~30@ a3~26@)))
                    (Sub (Mul (Mul b3~28@ a3~26@) (Mul d3~32@ c3~30@)) (Mul (Mul 24 a3~26@) (Mul d3~32@
                       c3~30@
                 ))))))
                 (=>
                  (= temp_3_1~1146@ (Mul (Sub (Sub (Mul (Mul d3~32@ b3~28@) (Mul a3~26@ c3~30@)) (Mul (Mul
                        b3~28@ b3~28@
                       ) (Mul d3~32@ d3~32@)
                      )
                     ) (Mul (Mul (Sub b3~28@ (Add c3~30@ c3~30@)) (Mul a3~26@ b3~28@)) (Mul a3~26@ c3~30@))
                    ) (Mul (Mul (Mul b3~28@ (Mul c3~30@ a3~26@)) (Add (Mul a3~26@ c3~30@) (Mul c3~30@ a3~26@)))
                     (Sub (Mul (Mul b3~28@ a3~26@) (Mul d3~32@ c3~30@)) (Mul (Mul 24 a3~26@) (Mul d3~32@
                        c3~30@
                  ))))))
                  (and
                   (=>
                    (= tmp%5@ (Sub b3~28@ (Add c3~30@ c3~30@)))
                    (=>
                     (= tmp%6@ (Mul a3~26@ b3~28@))
                     (=>
                      (= tmp%7@ (Mul a3~26@ c3~30@))
                      (=>
                       (ens%main!nl_basics.lemma_mul_is_associative. tmp%5@ tmp%6@ tmp%7@)
                       (=>
                        %%location_label%%3
                        (= temp_3_0~1013@ temp_3_1~1146@)
                   )))))
                   (=>
                    (= temp_3_0~1013@ temp_3_1~1146@)
                    (=>
                     (= temp_4_0~1325@ (Mul (Mul (Mul (Mul (Mul b4~36@ d4~40@) (Mul c4~38@ a4~34@)) (Mul (Sub
                           c4~38@ b4~36@
                          ) (Mul b4~36@ a4~34@)
                         )
                        ) (Mul (Sub (Mul b4~36@ c4~38@) (Mul b4~36@ d4~40@)) (Mul (Mul d4~40@ c4~38@) (Mul b4~36@
                           d4~40@
                        )))
                       ) (Mul (Mul (Mul (Mul a4~34@ a4~34@) (Mul d4~40@ 54)) (Mul d4~40@ (Mul c4~38@ b4~36@)))
                        (Mul b4~36@ (Mul (Mul b4~36@ 74) a4~34@))
                     )))
                     (=>
                      (= temp_4_1~1458@ (Mul (Mul (Mul (Mul (Mul b4~36@ d4~40@) (Mul c4~38@ a4~34@)) (Mul (Sub
                            c4~38@ b4~36@
                           ) (Mul b4~36@ a4~34@)
                          )
                         ) (Mul (Sub (Mul b4~36@ c4~38@) (Mul b4~36@ d4~40@)) (Mul (Mul d4~40@ c4~38@) (Mul b4~36@
                         ...
                         (Mul b4~36@ (Mul (Mul 74 b4~36@) a4~34@))
                      )))
                      (=>
                       (ens%main!nl_basics.lemma_mul_is_commutative. b4~36@ 74)
                       (=>
                        %%location_label%%4
                        (= temp_4_0~1325@ temp_4_1~1458@)
 )))))))))))))))))))))))
 (get-info :version)
 (assert
  %%query%%
