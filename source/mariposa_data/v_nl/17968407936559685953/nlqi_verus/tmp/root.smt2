...
;; mariposa_data/v_nl//17968407936559685953/nlqi_verus/src/main.rs:7:1: 26:40 (#0)
(push)
 ...
 (declare-const temp_1_0~425@ Int)
 (declare-const temp_1_1~526@ Int)
 (declare-const temp_2_0~675@ Int)
 (declare-const temp_2_1~796@ Int)
 (declare-const temp_3_0~957@ Int)
 (declare-const temp_3_1~1082@ Int)
 (declare-const temp_4_0~1275@ Int)
 (declare-const temp_4_1~1404@ Int)
 (assert
  fuel_defaults
 )
 ;; assertion failed
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~217@ (Add c0~6@ (Mul (Mul a0~2@ (Mul (Mul a0~2@ a0~2@) (Mul a0~2@ d0~8@)))
        (Mul (Mul (Mul d0~8@ a0~2@) (Mul d0~8@ d0~8@)) (Mul (Mul d0~8@ a0~2@) (Mul c0~6@ d0~8@)))
     )))
     (=>
      (= temp_0_1~274@ (Add c0~6@ (Mul (Mul a0~2@ (Mul (Mul a0~2@ a0~2@) (Mul a0~2@ d0~8@)))
         (Mul (Mul (Mul (Mul d0~8@ a0~2@) (Mul d0~8@ d0~8@)) (Mul d0~8@ a0~2@)) (Mul c0~6@ d0~8@))
      )))
      (and
       (=>
        (= tmp%1@ (Mul (Mul d0~8@ a0~2@) (Mul d0~8@ d0~8@)))
        (=>
         (= tmp%2@ (Mul d0~8@ a0~2@))
         (=>
          (= tmp%3@ (Mul c0~6@ d0~8@))
       ...
       (=>
        (= temp_0_0~217@ temp_0_1~274@)
        (=>
         (= temp_1_0~425@ (Add (Mul (Add (Add (Sub a1~10@ b1~12@) (Mul c1~14@ a1~10@)) (Mul (Mul
               b1~12@ c1~14@
              ) (Mul d1~16@ c1~14@)
             )
            ) (Mul d1~16@ (Mul (Mul a1~10@ a1~10@) (Mul a1~10@ b1~12@)))
           ) (Mul (Add (Mul a1~10@ (Mul c1~14@ c1~14@)) (Mul (Mul a1~10@ d1~16@) (Mul a1~10@ b1~12@)))
            (Mul (Add (Mul a1~10@ a1~10@) (Mul c1~14@ a1~10@)) b1~12@)
         )))
         (=>
          (= temp_1_1~526@ (Add (Mul (Add (Add (Sub a1~10@ b1~12@) (Mul c1~14@ a1~10@)) (Mul (Mul
                b1~12@ c1~14@
               ) (Mul d1~16@ c1~14@)
              )
             ) (Mul d1~16@ (Mul (Mul a1~10@ a1~10@) (Mul a1~10@ b1~12@)))
          ...
          (and
           (=>
            (ens%main!nl_basics.lemma_mul_is_commutative. a1~10@ a1~10@)
            (=>
             %%location_label%%1
             (= temp_1_0~425@ temp_1_1~526@)
           ))
           (=>
            (= temp_1_0~425@ temp_1_1~526@)
            (=>
             (= temp_2_0~675@ (Mul (Mul (Mul (Mul (Mul d2~24@ b2~20@) (Mul d2~24@ b2~20@)) (Add (Mul
                   c2~22@ c2~22@
                  ) (Add b2~20@ b2~20@)
                 )
                ) (Sub (Sub (Sub b2~20@ a2~18@) (Mul c2~22@ a2~18@)) (Mul (Sub c2~22@ b2~20@) d2~24@))
               ) (Mul (Mul (Mul (Sub d2~24@ b2~20@) (Mul b2~20@ b2~20@)) (Mul (Mul a2~18@ a2~18@) (
                   Mul d2~24@ c2~22@
                 ))
                ) (Mul (Mul (Mul d2~24@ c2~22@) b2~20@) (Mul (Sub a2~18@ c2~22@) (Mul d2~24@ b2~20@)))
             )))
             (=>
              (= temp_2_1~796@ (Mul (Mul (Mul (Mul (Mul d2~24@ b2~20@) (Mul d2~24@ b2~20@)) (Add (Mul
                    c2~22@ c2~22@
                   ) (Add b2~20@ b2~20@)
                  )
                 ) (Sub (Sub (Sub b2~20@ a2~18@) (Mul c2~22@ a2~18@)) (Mul (Sub c2~22@ b2~20@) d2~24@))
              ...
              (and
               (=>
                (= tmp%4@ (Mul d2~24@ b2~20@))
                (=>
                 (= tmp%5@ (Mul d2~24@ b2~20@))
                 (=>
                  (ens%main!nl_basics.lemma_mul_is_commutative. tmp%4@ tmp%5@)
                  (=>
                   %%location_label%%2
                   (= temp_2_0~675@ temp_2_1~796@)
               ))))
               (=>
                (= temp_2_0~675@ temp_2_1~796@)
                (=>
                 (= temp_3_0~957@ (Sub (Mul (Mul (Mul (Mul a3~26@ b3~28@) (Mul 70 a3~26@)) (Mul (Mul b3~28@
                       c3~30@
                      ) (Mul d3~32@ a3~26@)
                     )
                    ) (Mul (Mul (Mul c3~30@ a3~26@) (Mul a3~26@ b3~28@)) a3~26@)
                   ) (Mul (Mul (Mul d3~32@ (Mul c3~30@ a3~26@)) (Mul (Mul b3~28@ a3~26@) (Sub a3~26@ c3~30@)))
                    (Mul (Mul (Mul c3~30@ a3~26@) (Mul d3~32@ b3~28@)) (Mul (Mul a3~26@ b3~28@) (Mul b3~28@
                       a3~26@
                 ))))))
                 (=>
                  (= temp_3_1~1082@ (Sub (Mul (Mul (Mul (Mul b3~28@ c3~30@) (Mul d3~32@ a3~26@)) (Mul (Mul
                        a3~26@ b3~28@
                       ) (Mul 70 a3~26@)
                      )
                     ) (Mul (Mul (Mul c3~30@ a3~26@) (Mul a3~26@ b3~28@)) a3~26@)
                  ...
                  (and
                   (=>
                    (= tmp%6@ (Mul (Mul a3~26@ b3~28@) (Mul 70 a3~26@)))
                    (=>
                     (= tmp%7@ (Mul (Mul b3~28@ c3~30@) (Mul d3~32@ a3~26@)))
                     (=>
                      (ens%main!nl_basics.lemma_mul_is_commutative. tmp%6@ tmp%7@)
                      (=>
                       %%location_label%%3
                       (= temp_3_0~957@ temp_3_1~1082@)
                   ))))
                   (=>
                    (= temp_3_0~957@ temp_3_1~1082@)
                    (=>
                     (= temp_4_0~1275@ (Mul (Mul (Mul (Mul (Mul c4~38@ a4~34@) (Mul c4~38@ d4~40@)) (Mul (Mul
                           d4~40@ a4~34@
                          ) (Mul c4~38@ b4~36@)
                         )
                        ) (Add d4~40@ (Mul (Mul b4~36@ c4~38@) (Add c4~38@ b4~36@)))
                       ) (Mul (Mul (Mul (Sub a4~34@ c4~38@) (Mul a4~34@ a4~34@)) (Mul (Mul d4~40@ d4~40@) (
                           Mul b4~36@ c4~38@
                         ))
                        ) (Add (Add (Add 45 b4~36@) (Sub a4~34@ c4~38@)) (Mul (Mul a4~34@ c4~38@) (Mul a4~34@
                           b4~36@
                     ))))))
                     (=>
                      (= temp_4_1~1404@ (Mul (Mul (Mul (Mul (Mul c4~38@ a4~34@) (Mul c4~38@ d4~40@)) (Mul (Mul
                            d4~40@ a4~34@
                           ) (Mul c4~38@ b4~36@)
                          )
                         ) (Add d4~40@ (Mul (Mul b4~36@ c4~38@) (Add c4~38@ b4~36@)))
                        ) (Mul (Mul (Mul (Sub a4~34@ c4~38@) (Mul a4~34@ a4~34@)) (Mul (Mul (Mul d4~40@ d4~40@)
                            b4~36@
                           ) c4~38@
                          )
                         ) (Add (Add (Add 45 b4~36@) (Sub a4~34@ c4~38@)) (Mul (Mul a4~34@ c4~38@) (Mul a4~34@
                      ...
                      (=>
                       (= tmp%8@ (Mul d4~40@ d4~40@))
                       (=>
                        (ens%main!nl_basics.lemma_mul_is_associative. tmp%8@ b4~36@ c4~38@)
                        (=>
                         %%location_label%%4
                         (= temp_4_0~1275@ temp_4_1~1404@)
 ))))))))))))))))))))))))
 (get-info :version)
 (assert
  %%query%%
