...
;; mariposa_data/v_nl//15212990740753092799/nlqi_verus/src/main.rs:7:1: 26:40 (#0)
(push)
 ...
 (declare-const temp_1_0~551@ Int)
 (declare-const temp_1_1~660@ Int)
 (declare-const temp_2_0~797@ Int)
 (declare-const temp_2_1~894@ Int)
 (declare-const temp_3_0~1041@ Int)
 (declare-const temp_3_1~1154@ Int)
 (declare-const temp_4_0~1323@ Int)
 (declare-const temp_4_1~1444@ Int)
 (declare-const temp_5_0~1645@ Int)
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~277@ (Mul (Mul (Add (Sub (Add a0~2@ c0~6@) (Mul a0~2@ a0~2@)) (Mul (Mul a0~2@
           a0~2@
          ) (Add b0~4@ a0~2@)
         )
        ) (Add (Mul (Mul b0~4@ b0~4@) (Mul c0~6@ b0~4@)) (Mul c0~6@ (Mul d0~8@ a0~2@)))
       ) (Sub (Mul (Mul (Mul c0~6@ d0~8@) (Mul c0~6@ d0~8@)) (Add (Sub c0~6@ a0~2@) b0~4@))
        (Mul a0~2@ (Mul b0~4@ (Mul c0~6@ 70)))
     )))
     (=>
      (= temp_0_1~394@ (Mul (Mul (Add (Sub (Add a0~2@ c0~6@) (Mul a0~2@ a0~2@)) (Mul (Mul a0~2@
            a0~2@
           ) (Add b0~4@ a0~2@)
          )
         ) (Add (Mul (Mul b0~4@ b0~4@) (Mul c0~6@ b0~4@)) (Mul c0~6@ (Mul d0~8@ a0~2@)))
        ) (Sub (Mul (Mul (Mul c0~6@ d0~8@) (Mul c0~6@ d0~8@)) (Add (Sub c0~6@ a0~2@) b0~4@))
         (Mul (Mul b0~4@ (Mul c0~6@ 70)) a0~2@)
      )))
      (and
       (=>
        (= tmp%1@ (Mul b0~4@ (Mul c0~6@ 70)))
        (=>
         (ens%main!nl_basics.lemma_mul_is_commutative. a0~2@ tmp%1@)
         (=>
          %%location_label%%0
       ...
       (=>
        (= temp_0_0~277@ temp_0_1~394@)
        (=>
         (= temp_1_0~551@ (Mul (Sub (Mul (Mul (Mul c1~14@ d1~16@) (Mul a1~10@ b1~12@)) (Mul (Mul
               d1~16@ a1~10@
              ) (Mul c1~14@ a1~10@)
             )
            ) a1~10@
           ) (Mul (Mul (Mul (Mul c1~14@ c1~14@) (Mul a1~10@ a1~10@)) (Mul (Add a1~10@ 8) (Mul c1~14@
               a1~10@
             ))
            ) (Add (Mul (Sub b1~12@ d1~16@) (Mul b1~12@ c1~14@)) (Add c1~14@ (Mul c1~14@ c1~14@)))
         )))
         (=>
          (= temp_1_1~660@ (Mul (Sub (Mul (Mul (Mul c1~14@ d1~16@) (Mul a1~10@ b1~12@)) (Mul (Mul
                d1~16@ a1~10@
               ) (Mul c1~14@ a1~10@)
              )
             ) a1~10@
            ) (Mul (Mul (Mul (Mul c1~14@ c1~14@) (Mul a1~10@ a1~10@)) (Mul (Mul c1~14@ a1~10@) (
                Add a1~10@ 8
              ))
             ) (Add (Mul (Sub b1~12@ d1~16@) (Mul b1~12@ c1~14@)) (Add c1~14@ (Mul c1~14@ c1~14@)))
          )))
          (and
           (=>
            (= tmp%2@ (Add a1~10@ 8))
            (=>
             (= tmp%3@ (Mul c1~14@ a1~10@))
             (=>
              (ens%main!nl_basics.lemma_mul_is_commutative. tmp%2@ tmp%3@)
              (=>
               %%location_label%%1
               (= temp_1_0~551@ temp_1_1~660@)
           ))))
           (=>
            (= temp_1_0~551@ temp_1_1~660@)
            (=>
             (= temp_2_0~797@ (Add (Mul (Mul (Mul d2~24@ (Mul a2~18@ d2~24@)) (Mul (Add c2~22@ c2~22@)
                  (Mul b2~20@ b2~20@)
                 )
                ) (Mul (Mul (Add d2~24@ c2~22@) (Sub a2~18@ c2~22@)) (Add a2~18@ (Mul b2~20@ c2~22@)))
               ) (Mul b2~20@ (Mul (Mul (Mul d2~24@ b2~20@) (Mul b2~20@ d2~24@)) (Mul b2~20@ (Mul b2~20@
                   d2~24@
             ))))))
             (=>
              (= temp_2_1~894@ (Add (Mul (Mul (Mul d2~24@ (Mul a2~18@ d2~24@)) (Mul (Add c2~22@ c2~22@)
                   (Mul b2~20@ b2~20@)
                  )
                 ) (Mul (Sub (Mul (Add d2~24@ c2~22@) a2~18@) (Mul (Add d2~24@ c2~22@) c2~22@)) (Add
                   a2~18@ (Mul b2~20@ c2~22@)
                 ))
                ) (Mul b2~20@ (Mul (Mul (Mul d2~24@ b2~20@) (Mul b2~20@ d2~24@)) (Mul b2~20@ (Mul b2~20@
                    d2~24@
              ))))))
              (and
               (=>
                (= tmp%4@ (Add d2~24@ c2~22@))
                (=>
                 (ens%main!nl_basics.lemma_mul_is_distributive. tmp%4@ a2~18@ c2~22@)
                 (=>
                  %%location_label%%2
                  (= temp_2_0~797@ temp_2_1~894@)
               )))
               (=>
                (= temp_2_0~797@ temp_2_1~894@)
                (=>
                 (= temp_3_0~1041@ (Mul (Add (Mul (Mul (Mul b3~28@ a3~26@) b3~28@) (Mul (Mul b3~28@ d3~32@)
                      (Mul d3~32@ b3~28@)
                     )
                    ) (Add (Mul (Mul c3~30@ a3~26@) (Mul d3~32@ b3~28@)) (Mul d3~32@ (Mul d3~32@ c3~30@)))
                   ) (Mul (Mul (Add c3~30@ (Add d3~32@ b3~28@)) (Mul a3~26@ (Mul d3~32@ c3~30@))) (Mul
                     (Add (Sub d3~32@ b3~28@) (Mul a3~26@ d3~32@)) (Mul (Mul c3~30@ b3~28@) (Mul d3~32@
                       a3~26@
                 ))))))
                 (=>
                  (= temp_3_1~1154@ (Mul (Add (Mul (Mul (Mul b3~28@ d3~32@) (Mul d3~32@ b3~28@)) (Mul (Mul
                        b3~28@ a3~26@
                       ) b3~28@
                      )
                     ) (Add (Mul (Mul c3~30@ a3~26@) (Mul d3~32@ b3~28@)) (Mul d3~32@ (Mul d3~32@ c3~30@)))
                  ...
                  (and
                   (=>
                    (= tmp%5@ (Mul (Mul b3~28@ a3~26@) b3~28@))
                    (=>
                     (= tmp%6@ (Mul (Mul b3~28@ d3~32@) (Mul d3~32@ b3~28@)))
                     (=>
                      (ens%main!nl_basics.lemma_mul_is_commutative. tmp%5@ tmp%6@)
                      (=>
                       %%location_label%%3
                       (= temp_3_0~1041@ temp_3_1~1154@)
                   ))))
                   (=>
                    (= temp_3_0~1041@ temp_3_1~1154@)
                    (=>
                     (= temp_4_0~1323@ (Mul (Mul (Mul (Mul (Mul d4~40@ 30) (Mul d4~40@ b4~36@)) (Mul (Mul a4~34@
                           c4~38@
                          ) (Sub c4~38@ d4~40@)
                         )
                        ) (Add (Mul (Mul c4~38@ d4~40@) d4~40@) b4~36@)
                       ) (Mul (Mul (Mul (Mul b4~36@ a4~34@) (Mul a4~34@ d4~40@)) (Mul (Mul a4~34@ a4~34@) c4~38@))
                        (Mul (Mul (Mul a4~34@ b4~36@) (Mul c4~38@ d4~40@)) (Mul (Mul a4~34@ b4~36@) (Mul c4~38@
                           c4~38@
                     ))))))
                     (=>
                      (= temp_4_1~1444@ (Mul (Mul (Mul (Mul (Mul d4~40@ 30) (Mul d4~40@ b4~36@)) (Mul (Mul a4~34@
                            c4~38@
                           ) (Sub c4~38@ d4~40@)
                          )
                         ) (Add (Mul (Mul c4~38@ d4~40@) d4~40@) b4~36@)
                        ) (Mul (Mul (Mul (Mul b4~36@ a4~34@) (Mul a4~34@ d4~40@)) (Mul (Mul a4~34@ a4~34@) c4~38@))
                         (Mul (Mul (Mul a4~34@ b4~36@) (Mul c4~38@ d4~40@)) (Mul (Mul c4~38@ c4~38@) (Mul a4~34@
                            b4~36@
                      ))))))
                      (and
                       (=>
                        ...
                        (=>
                         (= tmp%8@ (Mul c4~38@ c4~38@))
                         (=>
                          (ens%main!nl_basics.lemma_mul_is_commutative. tmp%7@ tmp%8@)
                          (=>
                           %%location_label%%4
                           (= temp_4_0~1323@ temp_4_1~1444@)
                       ))))
                       (=>
                        (= temp_4_0~1323@ temp_4_1~1444@)
                        (=>
                         (= temp_5_0~1645@ (Add (Sub (Sub (Mul (Add b5~44@ a5~42@) (Mul b5~44@ a5~42@)) (Mul (Mul
                               b5~44@ a5~42@
                              ) (Mul b5~44@ b5~44@)
                             )
                            ) (Mul (Add (Mul b5~44@ d5~48@) (Mul b5~44@ 40)) (Mul (Mul b5~44@ c5~46@) (Mul c5~46@
                               b5~44@
                            )))
                           ) (Mul (Add (Mul (Mul b5~44@ b5~44@) (Add d5~48@ d5~48@)) (Mul (Mul b5~44@ b5~44@) (
                               Mul b5~44@ c5~46@
                             ))
                            ) (Mul (Mul (Mul b5~44@ 31) (Mul c5~46@ c5~46@)) (Add (Add d5~48@ c5~46@) (Mul 68 a5~42@)))
                         )))
                         (=>
                          (= temp_5_1~1810@ (Add (Sub (Sub (Mul (Add b5~44@ a5~42@) (Mul b5~44@ a5~42@)) (Mul (Mul
                                b5~44@ a5~42@
                               ) (Mul b5~44@ b5~44@)
                              )
                             ) (Mul (Add (Mul b5~44@ d5~48@) (Mul b5~44@ 40)) (Mul (Mul (Mul b5~44@ c5~46@) c5~46@)
                               b5~44@
                             ))
                            ) (Mul (Add (Mul (Mul b5~44@ b5~44@) (Add d5~48@ d5~48@)) (Mul (Mul b5~44@ b5~44@) (
                                Mul b5~44@ c5~46@
                          ...
                          (=>
                           (= tmp%9@ (Mul b5~44@ c5~46@))
                           (=>
                            (ens%main!nl_basics.lemma_mul_is_associative. tmp%9@ c5~46@ b5~44@)
                            (=>
                             %%location_label%%5
                             (= temp_5_0~1645@ temp_5_1~1810@)
 ))))))))))))))))))))))))))))
 (get-info :version)
 (assert
  %%query%%
