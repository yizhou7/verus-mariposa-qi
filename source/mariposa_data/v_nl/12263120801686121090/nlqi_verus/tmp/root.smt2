...
;; mariposa_data/v_nl//12263120801686121090/nlqi_verus/src/main.rs:7:1: 26:40 (#0)
(push)
 ...
 (declare-const temp_2_0~849@ Int)
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~281@ (Mul (Mul (Sub (Mul (Sub d0~8@ a0~2@) (Mul c0~6@ c0~6@)) (Mul (Mul a0~2@
           d0~8@
          ) (Mul a0~2@ c0~6@)
         )
        ) (Mul (Mul (Add c0~6@ a0~2@) (Add b0~4@ c0~6@)) b0~4@)
       ) (Mul (Mul (Mul (Mul c0~6@ d0~8@) (Mul a0~2@ 87)) (Mul (Mul b0~4@ c0~6@) (Add b0~4@
           c0~6@
         ))
        ) (Mul (Sub (Mul d0~8@ b0~4@) b0~4@) (Mul b0~4@ (Mul b0~4@ d0~8@)))
     )))
     (=>
      (= temp_0_1~402@ (Mul (Mul (Sub (Mul (Sub d0~8@ a0~2@) (Mul c0~6@ c0~6@)) (Mul (Mul a0~2@
            d0~8@
           ) (Mul a0~2@ c0~6@)
          )
         ) (Mul (Mul (Add c0~6@ a0~2@) (Add b0~4@ c0~6@)) b0~4@)
         ...
         ) (Mul (Mul b0~4@ (Mul b0~4@ d0~8@)) (Sub (Mul d0~8@ b0~4@) b0~4@))
      )))
      (and
       (=>
        (= tmp%1@ (Sub (Mul d0~8@ b0~4@) b0~4@))
        (=>
         (= tmp%2@ (Mul b0~4@ (Mul b0~4@ d0~8@)))
         (=>
       ...
       (=>
        (= temp_0_0~281@ temp_0_1~402@)
        (=>
         (= temp_1_0~591@ (Mul (Add (Add (Sub (Mul a1~10@ a1~10@) (Mul a1~10@ 24)) (Mul c1~14@
              (Mul d1~16@ b1~12@)
             )
            ) (Mul c1~14@ (Mul (Mul b1~12@ b1~12@) (Mul c1~14@ a1~10@)))
           ) (Mul (Sub (Mul (Mul b1~12@ c1~14@) (Mul d1~16@ 57)) (Add (Mul c1~14@ b1~12@) (Add 36
               c1~14@
             ))
            ) (Mul (Mul (Mul c1~14@ c1~14@) d1~16@) (Mul (Mul d1~16@ d1~16@) (Mul d1~16@ c1~14@)))
         )))
         (=>
          (= temp_1_1~736@ (Mul (Add (Add (Sub (Mul a1~10@ a1~10@) (Mul a1~10@ 24)) (Mul c1~14@
               (Mul d1~16@ b1~12@)
              )
             ) (Mul c1~14@ (Mul (Mul b1~12@ b1~12@) (Mul c1~14@ a1~10@)))
            ) (Mul (Sub (Mul (Mul b1~12@ c1~14@) (Mul d1~16@ 57)) (Add (Mul c1~14@ b1~12@) (Add 36
          ...
          (and
           (=>
            ...
            (=>
             %%location_label%%1
             (= temp_1_0~591@ temp_1_1~736@)
           ))
           (=>
            (= temp_1_0~591@ temp_1_1~736@)
            (=>
             (= temp_2_0~849@ (Add (Add (Sub c2~22@ (Mul c2~22@ (Mul c2~22@ a2~18@))) (Mul (Mul (Add
                   c2~22@ b2~20@
                  ) (Mul c2~22@ c2~22@)
                 ) (Mul (Sub d2~24@ a2~18@) d2~24@)
                )
               ) (Mul (Mul c2~22@ (Mul (Mul c2~22@ a2~18@) d2~24@)) (Mul (Add (Mul d2~24@ c2~22@) (
                   Mul d2~24@ b2~20@
                  )
                 ) (Mul d2~24@ c2~22@)
             ))))
             (=>
              (= temp_2_1~934@ (Add (Add (Sub c2~22@ (Mul c2~22@ (Mul c2~22@ a2~18@))) (Mul (Mul (Add
                    c2~22@ b2~20@
                   ) (Mul c2~22@ c2~22@)
                  ) (Mul (Sub d2~24@ a2~18@) d2~24@)
                 )
                ) (Mul (Mul c2~22@ (Mul c2~22@ (Mul a2~18@ d2~24@))) (Mul (Add (Mul d2~24@ c2~22@) (
                    Mul d2~24@ b2~20@
                   )
                  ) (Mul d2~24@ c2~22@)
              ))))
              (=>
               (ens%main!nl_basics.lemma_mul_is_associative. c2~22@ a2~18@ d2~24@)
               (=>
                %%location_label%%2
                (= temp_2_0~849@ temp_2_1~934@)
 )))))))))))))))
 (get-info :version)
 (assert
  %%query%%
