...
(set-info :comment ";; mariposa_data/v_nl//5508863490063391896/nlqi_verus/src/main.rs:7:1: 36:40 (#0)")
(push 1)
...
(assert (=> %%query%% (not (=> (= temp_0_0~277@ (Mul b0~4@ (Mul (Mul (Sub b0~4@ a0~2@) (Mul c0~6@ d0~8@)) (Mul (Mul a0~2@ d0~8@) (Mul a0~2@ b0~4@))))) (=> (= temp_0_1~314@ (Mul b0~4@ (Mul (Mul (Mul (Sub b0~4@ a0~2@) c0~6@) d0~8@) (Mul (Mul a0~2@ d0~8@) (Mul a0~2@ b0~4@))))) (=> (= tmp%1@ (Sub b0~4@ a0~2@)) (=> (ens%main!nl_basics.lemma_mul_is_associative. tmp%1@ c0~6@ d0~8@) (=> %%location_label%%0 (= temp_0_0~277@ temp_0_1~314@)))))))))
(assert %%query%%)
(check-sat)
