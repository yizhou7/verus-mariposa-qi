...
;; mariposa_data/v_nl//13452356439655812593/nlqi_verus/src/main.rs:7:1: 36:40 (#0)
(push)
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~305@ (Mul (Mul (Mul (Mul c0~6@ c0~6@) (Mul c0~6@ d0~8@)) (Mul (Mul b0~4@ a0~2@)
         (Add a0~2@ c0~6@)
        )
       ) (Mul (Mul (Mul d0~8@ c0~6@) (Mul c0~6@ b0~4@)) (Mul (Mul b0~4@ b0~4@) (Mul a0~2@ d0~8@)))
     ))
     (=>
      (= temp_0_1~370@ (Mul (Mul (Mul (Mul c0~6@ c0~6@) (Mul c0~6@ d0~8@)) (Mul b0~4@ (Mul a0~2@
           (Add a0~2@ c0~6@)
         ))
        ) (Mul (Mul (Mul d0~8@ c0~6@) (Mul c0~6@ b0~4@)) (Mul (Mul b0~4@ b0~4@) (Mul a0~2@ d0~8@)))
      ))
      (=>
       ...
       (and
        ...
        (=>
         tmp%1@
         (=>
          (= temp_1_0~442@ (Add (Mul (Mul (Mul b1~12@ a1~10@) b1~12@) (Mul a1~10@ (Mul a1~10@ b1~12@)))
            (Mul (Mul (Mul c1~14@ b1~12@) (Mul a1~10@ b1~12@)) (Add (Mul d1~16@ c1~14@) (Sub d1~16@
               a1~10@
          )))))
          (=>
           (= temp_1_1~499@ (Add (Mul (Mul (Mul b1~12@ a1~10@) b1~12@) (Mul (Mul a1~10@ b1~12@)
               a1~10@
              )
             ) (Mul (Mul (Mul c1~14@ b1~12@) (Mul a1~10@ b1~12@)) (Add (Mul d1~16@ c1~14@) (Sub d1~16@
                a1~10@
           ...
           (=>
            ...
            (and
             ...
             (=>
              tmp%2@
              (=>
               (= temp_2_0~587@ (Add (Mul (Sub (Mul c2~22@ d2~24@) (Mul b2~20@ b2~20@)) (Mul (Mul d2~24@
                    34
                   ) d2~24@
                  )
                 ) (Mul (Mul (Mul c2~22@ d2~24@) (Mul d2~24@ b2~20@)) (Add (Mul d2~24@ d2~24@) (Add c2~22@
                    a2~18@
               )))))
               (=>
                (= temp_2_1~676@ (Add (Mul (Sub (Mul c2~22@ d2~24@) (Mul b2~20@ b2~20@)) (Mul (Mul d2~24@
                     34
                    ) d2~24@
                   )
                  ) (Add (Mul (Mul (Mul c2~22@ d2~24@) (Mul d2~24@ b2~20@)) (Mul d2~24@ d2~24@)) (Mul
                    (Mul (Mul c2~22@ d2~24@) (Mul d2~24@ b2~20@)) (Add c2~22@ a2~18@)
                ))))
                (=>
                 (= tmp%3@ (= temp_2_0~587@ temp_2_1~676@))
                 (=>
                  %%location_label%%2
                  tmp%3@
 )))))))))))))))))
