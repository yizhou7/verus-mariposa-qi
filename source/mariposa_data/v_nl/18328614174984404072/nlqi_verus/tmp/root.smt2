...
;; mariposa_data/v_nl//18328614174984404072/nlqi_verus/src/main.rs:7:1: 36:40 (#0)
(push)
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~293@ (Mul (Mul c0~6@ (Mul (Mul a0~2@ a0~2@) (Mul a0~2@ c0~6@))) (Mul (Mul
         (Mul a0~2@ c0~6@) (Sub a0~2@ c0~6@)
        ) (Add (Mul c0~6@ b0~4@) (Mul a0~2@ d0~8@))
     )))
     (=>
      (= temp_0_1~346@ (Mul (Mul c0~6@ (Mul (Mul a0~2@ a0~2@) (Mul a0~2@ c0~6@))) (Mul (Mul
          a0~2@ (Mul c0~6@ (Sub a0~2@ c0~6@))
         ) (Add (Mul c0~6@ b0~4@) (Mul a0~2@ d0~8@))
      )))
      (and
       (=>
       ...
       (=>
        (= temp_0_0~293@ temp_0_1~346@)
        (=>
         (= temp_1_0~431@ (Add (Add (Mul (Mul b1~12@ d1~16@) c1~14@) (Add (Mul a1~10@ d1~16@)
             (Add b1~12@ b1~12@)
            )
           ) (Mul (Mul (Mul b1~12@ a1~10@) (Sub b1~12@ c1~14@)) (Mul (Mul c1~14@ b1~12@) (Mul c1~14@
              c1~14@
         )))))
         (=>
          (= temp_1_1~500@ (Add (Add (Mul (Mul b1~12@ d1~16@) c1~14@) (Add (Mul a1~10@ d1~16@)
              (Add b1~12@ b1~12@)
             )
            ) (Mul (Sub (Mul (Mul b1~12@ a1~10@) b1~12@) (Mul (Mul b1~12@ a1~10@) c1~14@)) (Mul
              (Mul c1~14@ b1~12@) (Mul c1~14@ c1~14@)
          ))))
          (=>
           (ens%main!nl_basics.lemma_mul_properties_auto_1. 0)
           (=>
