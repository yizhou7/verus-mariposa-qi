...
;; mariposa_data/v_nl//16271168730428432596/nlqi_verus/src/main.rs:7:1: 26:40 (#0)
(push)
 ...
 (declare-const temp_1_0~729@ Int)
 (declare-const temp_1_1~858@ Int)
 (declare-const temp_2_0~1021@ Int)
 (declare-const temp_2_1~1150@ Int)
 (declare-const temp_3_0~1301@ Int)
 (declare-const temp_3_1~1418@ Int)
 (assert
  fuel_defaults
 )
 ;; assertion failed
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~321@ (Mul (Mul (Mul (Mul (Mul b0~4@ b0~4@) (Mul d0~8@ a0~2@)) (Mul c0~6@ (
           Mul c0~6@ a0~2@
         ))
        ) (Add (Mul (Mul b0~4@ 94) (Mul d0~8@ b0~4@)) (Mul (Mul d0~8@ a0~2@) (Mul d0~8@ b0~4@)))
       ) (Sub (Mul (Mul (Mul c0~6@ a0~2@) (Mul c0~6@ 57)) (Mul (Add c0~6@ a0~2@) (Mul d0~8@
           c0~6@
         ))
        ) (Mul (Mul (Mul a0~2@ d0~8@) (Mul d0~8@ 29)) (Mul (Sub c0~6@ b0~4@) (Mul b0~4@ c0~6@)))
     )))
     (=>
      (= temp_0_1~510@ (Mul (Add (Mul (Mul (Mul (Mul b0~4@ b0~4@) (Mul d0~8@ a0~2@)) (Mul c0~6@
            (Mul c0~6@ a0~2@)
           )
          ) (Mul (Mul b0~4@ 94) (Mul d0~8@ b0~4@))
         ) (Mul (Mul (Mul (Mul b0~4@ b0~4@) (Mul d0~8@ a0~2@)) (Mul c0~6@ (Mul c0~6@ a0~2@)))
          (Mul (Mul d0~8@ a0~2@) (Mul d0~8@ b0~4@))
         )
        ) (Sub (Mul (Mul (Mul c0~6@ a0~2@) (Mul c0~6@ 57)) (Mul (Add c0~6@ a0~2@) (Mul d0~8@
            c0~6@
          ))
      ...
      (and
       (=>
        (= tmp%1@ (Mul (Mul (Mul b0~4@ b0~4@) (Mul d0~8@ a0~2@)) (Mul c0~6@ (Mul c0~6@ a0~2@))))
        (=>
         (= tmp%2@ (Mul (Mul b0~4@ 94) (Mul d0~8@ b0~4@)))
         (=>
          (= tmp%3@ (Mul (Mul d0~8@ a0~2@) (Mul d0~8@ b0~4@)))
          (=>
           (ens%main!nl_basics.lemma_mul_is_distributive. tmp%1@ tmp%2@ tmp%3@)
           (=>
            %%location_label%%0
            (= temp_0_0~321@ temp_0_1~510@)
       )))))
       (=>
        (= temp_0_0~321@ temp_0_1~510@)
        (=>
         (= temp_1_0~729@ (Mul (Sub (Mul (Sub (Mul b1~12@ b1~12@) (Mul d1~16@ c1~14@)) (Add (Mul
               a1~10@ c1~14@
              ) (Mul a1~10@ c1~14@)
             )
            ) (Mul (Mul b1~12@ (Mul c1~14@ a1~10@)) (Mul (Sub b1~12@ a1~10@) (Mul d1~16@ a1~10@)))
           ) (Add (Mul (Add (Mul c1~14@ b1~12@) a1~10@) (Mul (Mul a1~10@ c1~14@) (Mul c1~14@ c1~14@)))
            (Mul (Sub (Mul 47 d1~16@) (Mul c1~14@ d1~16@)) (Mul c1~14@ (Mul b1~12@ c1~14@)))
         )))
         (=>
          (= temp_1_1~858@ (Mul (Sub (Mul (Sub (Mul b1~12@ b1~12@) (Mul d1~16@ c1~14@)) (Add (Mul
                a1~10@ c1~14@
               ) (Mul a1~10@ c1~14@)
              )
             ) (Mul (Mul b1~12@ (Mul c1~14@ a1~10@)) (Mul (Mul (Sub b1~12@ a1~10@) d1~16@) a1~10@))
            ) (Add (Mul (Add (Mul c1~14@ b1~12@) a1~10@) (Mul (Mul a1~10@ c1~14@) (Mul c1~14@ c1~14@)))
             (Mul (Sub (Mul 47 d1~16@) (Mul c1~14@ d1~16@)) (Mul c1~14@ (Mul b1~12@ c1~14@)))
          )))
          (and
           (=>
            (= tmp%4@ (Sub b1~12@ a1~10@))
            (=>
             (ens%main!nl_basics.lemma_mul_is_associative. tmp%4@ d1~16@ a1~10@)
             (=>
              %%location_label%%1
              (= temp_1_0~729@ temp_1_1~858@)
           )))
           (=>
            (= temp_1_0~729@ temp_1_1~858@)
            (=>
             (= temp_2_0~1021@ (Add (Mul (Mul (Mul (Add c2~22@ b2~20@) (Mul d2~24@ d2~24@)) (Sub (Mul
                   a2~18@ b2~20@
                  ) (Mul c2~22@ d2~24@)
                 )
                ) (Mul (Mul (Add d2~24@ d2~24@) (Mul c2~22@ d2~24@)) (Add (Mul b2~20@ b2~20@) (Mul a2~18@
                   b2~20@
                )))
               ) (Mul (Mul (Mul (Mul d2~24@ c2~22@) (Mul a2~18@ b2~20@)) (Mul (Mul b2~20@ a2~18@) (
                   Mul a2~18@ a2~18@
                 ))
                ) (Mul (Mul (Mul d2~24@ b2~20@) (Mul d2~24@ c2~22@)) (Mul (Mul c2~22@ c2~22@) (Mul b2~20@
                   a2~18@
             ))))))
             (=>
              (= temp_2_1~1150@ (Add (Mul (Mul (Mul (Add c2~22@ b2~20@) (Mul d2~24@ d2~24@)) (Sub (Mul
                    a2~18@ b2~20@
                   ) (Mul c2~22@ d2~24@)
                  )
                 ) (Mul (Mul (Add d2~24@ d2~24@) (Mul c2~22@ d2~24@)) (Add (Mul b2~20@ b2~20@) (Mul a2~18@
                 ...
                 ) (Mul (Mul (Mul d2~24@ b2~20@) (Mul d2~24@ c2~22@)) (Mul (Mul (Mul c2~22@ c2~22@) b2~20@)
                   a2~18@
              )))))
              (and
               (=>
                ...
                (=>
                 (ens%main!nl_basics.lemma_mul_is_associative. tmp%5@ b2~20@ a2~18@)
                 (=>
                  %%location_label%%2
                  (= temp_2_0~1021@ temp_2_1~1150@)
               )))
               (=>
                (= temp_2_0~1021@ temp_2_1~1150@)
                (=>
                 (= temp_3_0~1301@ (Mul (Mul (Mul (Mul (Mul c3~30@ d3~32@) (Mul b3~28@ d3~32@)) (Mul (Mul
                       d3~32@ c3~30@
                      ) (Mul d3~32@ b3~28@)
                     )
                    ) (Mul (Mul (Mul d3~32@ c3~30@) (Mul a3~26@ d3~32@)) (Add (Mul a3~26@ b3~28@) (Mul a3~26@
                       a3~26@
                    )))
                   ) (Sub (Sub (Mul (Mul a3~26@ b3~28@) (Sub c3~30@ d3~32@)) (Mul (Mul d3~32@ a3~26@) (
                       Sub d3~32@ b3~28@
                     ))
                    ) (Mul (Add (Mul a3~26@ c3~30@) (Mul c3~30@ d3~32@)) b3~28@)
                 )))
                 (=>
                  (= temp_3_1~1418@ (Mul (Mul (Mul (Mul (Mul c3~30@ d3~32@) (Mul b3~28@ d3~32@)) (Mul (Mul
                        d3~32@ c3~30@
                       ) (Mul d3~32@ b3~28@)
                      )
                     ) (Mul (Mul (Mul d3~32@ c3~30@) (Mul a3~26@ d3~32@)) (Add (Mul a3~26@ b3~28@) (Mul a3~26@
                    ...
                    ) (Sub (Sub (Mul (Mul a3~26@ b3~28@) (Sub c3~30@ d3~32@)) (Mul d3~32@ (Mul a3~26@ (Sub
                         d3~32@ b3~28@
                      )))
                     ) (Mul (Add (Mul a3~26@ c3~30@) (Mul c3~30@ d3~32@)) b3~28@)
                  )))
                  (=>
                   (= tmp%6@ (Sub d3~32@ b3~28@))
                   (=>
                    (ens%main!nl_basics.lemma_mul_is_associative. d3~32@ a3~26@ tmp%6@)
                    (=>
                     %%location_label%%3
                     (= temp_3_0~1301@ temp_3_1~1418@)
 ))))))))))))))))))))
 (get-info :version)
 (assert
  %%query%%
