...
;; mariposa_data/v_nl//16227361275643094056/nlqi_verus/src/main.rs:7:1: 26:40 (#0)
(push)
 ...
 (declare-const temp_1_0~655@ Int)
 (declare-const temp_1_1~792@ Int)
 (declare-const temp_2_0~969@ Int)
 (declare-const temp_2_1~1118@ Int)
 (declare-const temp_3_0~1271@ Int)
 (declare-const temp_3_1~1396@ Int)
 (assert
  fuel_defaults
 )
 ;; assertion failed
 ...
 (assert
  (=>
   %%query%%
   (not (=>
     (= temp_0_0~293@ (Mul (Mul (Sub (Mul (Mul d0~8@ b0~4@) (Mul a0~2@ a0~2@)) (Mul c0~6@ (
           Sub d0~8@ d0~8@
         ))
        ) (Mul (Mul (Mul d0~8@ a0~2@) (Mul b0~4@ d0~8@)) (Mul (Mul b0~4@ a0~2@) (Mul d0~8@ c0~6@)))
       ) (Mul (Mul (Add (Mul b0~4@ 36) (Sub c0~6@ c0~6@)) (Mul (Mul b0~4@ c0~6@) (Mul b0~4@
           d0~8@
         ))
        ) (Mul (Mul (Mul d0~8@ c0~6@) d0~8@) (Mul (Mul b0~4@ b0~4@) (Mul c0~6@ d0~8@)))
     )))
     (=>
      (= temp_0_1~426@ (Mul (Mul (Sub (Mul (Mul d0~8@ b0~4@) (Mul a0~2@ a0~2@)) (Mul c0~6@ (
            Sub d0~8@ d0~8@
          ))
         ) (Mul (Mul (Mul d0~8@ a0~2@) (Mul b0~4@ d0~8@)) (Mul (Mul b0~4@ a0~2@) (Mul d0~8@ c0~6@)))
        ) (Mul (Mul (Mul (Mul d0~8@ c0~6@) d0~8@) (Mul (Mul b0~4@ b0~4@) (Mul c0~6@ d0~8@)))
         (Mul (Add (Mul b0~4@ 36) (Sub c0~6@ c0~6@)) (Mul (Mul b0~4@ c0~6@) (Mul b0~4@ d0~8@)))
      )))
      (and
       (=>
        (= tmp%1@ (Mul (Add (Mul b0~4@ 36) (Sub c0~6@ c0~6@)) (Mul (Mul b0~4@ c0~6@) (Mul b0~4@
            d0~8@
        ))))
        (=>
         (= tmp%2@ (Mul (Mul (Mul d0~8@ c0~6@) d0~8@) (Mul (Mul b0~4@ b0~4@) (Mul c0~6@ d0~8@))))
         (=>
       ...
       (=>
        (= temp_0_0~293@ temp_0_1~426@)
        (=>
         (= temp_1_0~655@ (Mul (Mul (Mul (Mul (Mul a1~10@ c1~14@) (Mul a1~10@ a1~10@)) (Add (Sub
               a1~10@ d1~16@
              ) (Mul a1~10@ c1~14@)
             )
            ) (Mul (Mul c1~14@ (Sub d1~16@ b1~12@)) (Mul (Mul b1~12@ b1~12@) (Mul d1~16@ b1~12@)))
           ) (Mul (Sub (Mul (Sub a1~10@ d1~16@) (Mul c1~14@ c1~14@)) (Mul (Mul b1~12@ d1~16@) (
               Mul d1~16@ c1~14@
             ))
            ) (Mul (Mul (Add d1~16@ b1~12@) (Mul 4 d1~16@)) (Mul (Mul c1~14@ a1~10@) (Add c1~14@
               c1~14@
         ))))))
         (=>
          (= temp_1_1~792@ (Mul (Mul (Mul (Mul (Mul a1~10@ c1~14@) (Mul a1~10@ a1~10@)) (Add (Sub
                a1~10@ d1~16@
               ) (Mul a1~10@ c1~14@)
              )
             ) (Mul (Mul c1~14@ (Sub d1~16@ b1~12@)) (Mul (Mul b1~12@ b1~12@) (Mul d1~16@ b1~12@)))
          ...
          (and
           (=>
            ...
            (=>
             %%location_label%%1
             (= temp_1_0~655@ temp_1_1~792@)
           ))
           (=>
            (= temp_1_0~655@ temp_1_1~792@)
            (=>
             (= temp_2_0~969@ (Mul (Mul (Mul (Mul (Mul d2~24@ d2~24@) c2~22@) (Mul (Mul d2~24@ 78)
                  (Mul c2~22@ a2~18@)
                 )
                ) (Mul (Sub (Mul b2~20@ c2~22@) (Mul b2~20@ c2~22@)) (Mul (Mul b2~20@ c2~22@) (Mul d2~24@
                   d2~24@
                )))
               ) (Mul (Mul (Mul (Mul c2~22@ c2~22@) (Sub b2~20@ a2~18@)) (Mul (Mul c2~22@ c2~22@) (
                   Mul c2~22@ d2~24@
                 ))
                ) (Mul (Mul (Mul c2~22@ d2~24@) (Mul c2~22@ c2~22@)) (Mul (Mul c2~22@ a2~18@) (Mul a2~18@
                   7
             ))))))
             (=>
              (= temp_2_1~1118@ (Mul (Mul (Mul (Mul (Mul d2~24@ d2~24@) c2~22@) (Mul (Mul d2~24@ 78)
                   (Mul c2~22@ a2~18@)
                  )
                 ) (Mul (Sub (Mul b2~20@ c2~22@) (Mul b2~20@ c2~22@)) (Mul (Mul b2~20@ c2~22@) (Mul d2~24@
                    d2~24@
                 ...
                 ) (Mul (Mul (Mul c2~22@ d2~24@) (Mul c2~22@ c2~22@)) (Mul (Mul a2~18@ c2~22@) (Mul a2~18@
                    7
              ))))))
              (and
               (=>
                (ens%main!nl_basics.lemma_mul_is_commutative. c2~22@ a2~18@)
                (=>
                 %%location_label%%2
                 (= temp_2_0~969@ temp_2_1~1118@)
               ))
               (=>
                (= temp_2_0~969@ temp_2_1~1118@)
                (=>
                 (= temp_3_0~1271@ (Add (Add (Mul (Mul (Add d3~32@ b3~28@) (Mul d3~32@ c3~30@)) (Mul (Mul
                       c3~30@ a3~26@
                      ) (Mul a3~26@ b3~28@)
                     )
                    ) (Mul (Mul (Mul d3~32@ a3~26@) (Mul a3~26@ d3~32@)) (Mul (Mul c3~30@ c3~30@) (Sub c3~30@
                       d3~32@
                    )))
                   ) (Sub (Add (Mul (Sub c3~30@ d3~32@) d3~32@) (Mul (Sub a3~26@ c3~30@) (Mul b3~28@ c3~30@)))
                    (Mul (Mul (Mul a3~26@ b3~28@) (Mul c3~30@ a3~26@)) (Add (Mul d3~32@ a3~26@) (Sub c3~30@
                       b3~28@
                 ))))))
                 (=>
                  (= temp_3_1~1396@ (Add (Add (Mul (Mul (Mul (Add d3~32@ b3~28@) d3~32@) c3~30@) (Mul (Mul
                        c3~30@ a3~26@
                       ) (Mul a3~26@ b3~28@)
                      )
                     ) (Mul (Mul (Mul d3~32@ a3~26@) (Mul a3~26@ d3~32@)) (Mul (Mul c3~30@ c3~30@) (Sub c3~30@
                  ...
                  (=>
                   (= tmp%3@ (Add d3~32@ b3~28@))
                   (=>
                    (ens%main!nl_basics.lemma_mul_is_associative. tmp%3@ d3~32@ c3~30@)
                    (=>
                     %%location_label%%3
                     (= temp_3_0~1271@ temp_3_1~1396@)
 ))))))))))))))))))))
 (get-info :version)
 (assert
  %%query%%
