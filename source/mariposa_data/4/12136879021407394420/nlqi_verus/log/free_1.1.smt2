...
(assert (=> %%query%% (not (=> (ens%main!nl_basics.lemma_mul_properties_auto_1. 0) (=> (= temp_0_0~145@ (Mul (Mul (Mul (Mul a0~2@ b0~4@) (Mul a0~2@ c0~6@)) (Mul d0~8@ (Sub c0~6@ b0~4@))) (Mul (Mul (Mul c0~6@ b0~4@) (Add c0~6@ b0~4@)) (Mul (Mul b0~4@ a0~2@) (Add c0~6@ c0~6@))))) (=> (= temp_0_1~206@ (Mul (Mul (Mul a0~2@ b0~4@) (Mul a0~2@ c0~6@)) (Mul (Mul d0~8@ (Sub c0~6@ b0~4@)) (Mul (Mul (Mul c0~6@ b0~4@) (Add c0~6@ b0~4@)) (Mul (Mul b0~4@ a0~2@) (Add c0~6@ c0~6@)))))) (=> (= tmp%1@ (= temp_0_0~145@ temp_0_1~206@)) (=> %%location_label%%0 tmp%1@))))))))
(assert %%query%%)
(check-sat)
