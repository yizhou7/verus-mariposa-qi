...
(assert (=> %%query%% (not (=> (ens%main!nl_basics.lemma_mul_properties_auto_1. 0) (=> (= temp_0_0~133@ (Add (Mul (Mul (Mul a0~2@ a0~2@) (Mul b0~4@ d0~8@)) b0~4@) (Mul (Mul (Mul c0~6@ a0~2@) (Mul b0~4@ d0~8@)) (Add c0~6@ (Sub a0~2@ b0~4@))))) (=> (= temp_0_1~182@ (Add (Mul (Mul (Mul a0~2@ a0~2@) (Mul b0~4@ d0~8@)) b0~4@) (Mul (Mul (Mul a0~2@ c0~6@) (Mul b0~4@ d0~8@)) (Add c0~6@ (Sub a0~2@ b0~4@))))) (=> (= tmp%1@ (= temp_0_0~133@ temp_0_1~182@)) (=> %%location_label%%0 tmp%1@))))))))
(assert %%query%%)
(check-sat)
